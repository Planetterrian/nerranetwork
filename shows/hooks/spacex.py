...
def pre_fetch(config, *, episode_num=None, today_str=None) -> dict:
    ...
    try:
        price, change_str = _fetch_spcx_quote()
...
def _fetch_spcx_quote() -> tuple[float, str]:
    """Return ``(price, change_str)`` or ``(0.0, "")`` when no source
    yields a quote that passes validation."""
