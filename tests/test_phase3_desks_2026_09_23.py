...
class TestDeskConfig:
    ...
    def test_pages_covers_and_launched(self, slug):
        ...
        # A scheduled show's pages are regenerated by run-show; a nightly
        # add-path for them is a stale pathspec (test_page_regen_ownership).
        nightly = (ROOT / ".github/workflows/nightly-maintenance.yml").read_text()
        assert f"blog/{slug}/**" not in nightly and f"{page}.html" not in nightly
