...
class TestFactualCorrections:
    ...
    def test_9_gig_work_sources(self):
        """BLS publishes no such worker estimate; its Contingent Worker
        Survey ran 2017 and again 2023 (the omnibus verification pass,
        4be6ee41, tightened the digest to say so — this anchor tracked
        it 2026-08-25). The MIT/CEEPR revision ($3.37 -> $8.55-10) is
        noted; the Uber-commissioned study is 2015 with a $16-30 median
        range."""
        t = _digest(69)
        assert "Estimates from the Bureau of Labor Statistics" not in t
        assert "contingent-worker survey" in t
