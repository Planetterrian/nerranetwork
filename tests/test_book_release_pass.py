...
class TestWO15AudiobookExportAndBooksPage:
    ...
    def test_live_volume_yamls_carry_the_new_keys(self):
        ...
        for vid, b2r in expect.items():
            ...
            # Sep 13 2026 (PR #1194): both audiobooks are on Payhip now, so
            # the key is filled rather than scaffolded-empty.
            assert links["audiobook_direct"].startswith("https://payhip.com/b/")
        # Render from the YAMLs instead of reading the committed page —
        # books.html is regenerated by the nightly, so the file lags the
        # YAML by up to a day and the assertion would flip with it.
        import generate_html as gh
        page = gh.render_books_page(
            gh.books_page_volumes(ROOT / "books" / "catalog.json", VOLS))
        assert page.count("books2read.com/u/") == 2
        assert "Buy direct" in page and page.count("Buy the audiobook") == 2
        # With audiobooks on Payhip the direct-sales note names both formats.
        assert "Buying direct gets you the format you choose" in page
